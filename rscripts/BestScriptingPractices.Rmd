...
# Tutorial {.tabset .tabset-fade .tabset-pills}
...
So, what is actually happening in this script? I just wrote it, so I know, in this moment, exactly what is going on. But will "future me" know? Future me is pretty forgetful and likes quick answers. As to not upset future me, it might be best to write them a few notes in the document right now. We can easily do that with a single (or more, if you like) hash mark (#) before comment text. See below how I integrate it into the script we just wrote.
...
I see a lot of redundancy in this code. What if I told you I could remove a third of the content in this script with one simple function? Repeating code to do the same thing over and over again will inevitably lead to issues and typos. Further, what if we suddenly decided we wanted all graphs to have a new title or change the colors? In the old script, we would have to change that in each plot individually. Now, with a function, we can change that once and it will be changed everywhere. 

Though this is a small example and this version of the code would really be sufficient, I am sure you can see how that could become a massive problem in a long script you have been working tirelessly on. (I'll save the below file so you can view it later, too).
